...
const normalizeMonthDates = (dates: Date[][]) => {
    return dates.map((week) =>
        week.map((date) => moment(date).startOf('day').toDate())
    );
}
...
export default function Calendar({dateStates = undefined, getSelectedDates = undefined, showTwoMonths = true, earlierMonthsSelectable = false} : Props) {
    ...
    let [initDate, setDate] = useState<Date>(moment().startOf('day').toDate());
    
    let [monthDate, setMonthDate] = useState<moment.Moment>(moment(initDate).startOf('month'))
    let [monthDates, setMonthDates] = useState<Date[][]>(normalizeMonthDates(calendar.monthDates(monthDate.year(), monthDate.month())))
    ...
    let [monthDate2, setMonthDate2] = useState<moment.Moment>(moment(initDate).add(1, 'month').startOf('month'))
    let [monthDates2, setMonthDates2] = useState<Date[][]>(normalizeMonthDates(calendar.monthDates(monthDate2.year(), monthDate2.month())))
    ...
    useEffect(() => {
        let md = moment(initDate).startOf('month')
        let mdts = normalizeMonthDates(calendar.monthDates(md.year(), md.month()))
        setMonthDate(md)
        setMonthDates(mdts)
        setMonthStart(md.startOf('month').toDate())
        setMonthEnd(md.endOf('month').toDate())

        let md2 = moment(initDate).add(1, 'month').startOf('month')
        let mdts2 = normalizeMonthDates(calendar.monthDates(md2.year(), md2.month()))

        if(mdts.length > mdts2.length){
            let newDates: Date[] = [];
            let start: Date = mdts2[mdts2.length-1][mdts2[mdts2.length-1].length-1]
            for(var x=0; x<7; x++){
                newDates.push(moment(start).add(1+x, 'days').startOf('day').toDate())
            }
            mdts2.push(newDates)
        }
            ...
            for(var x=0; x<7; x++){
                newDates2.push(moment(start2).add(1+x, 'days').startOf('day').toDate())
            }
            mdts.push(newDates2)
        }
