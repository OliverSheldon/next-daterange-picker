...
const CalendarMonth = createClass({
  ...
  renderDayHeaders() {
    ...
    let headers = indices.map(function(index) {
      ...
      return (
        <th className={this.cx({element: 'WeekdayHeading'})} key={weekday} scope="col"><abbr title={weekday && weekday.length > 0 ? weekday[0] : ''}>{weekday && weekday.length > 0 ? weekday[1] : ''}</abbr></th>
      );
    }.bind(this));
